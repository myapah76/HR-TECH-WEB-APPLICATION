...
export default function BulkAiScoringBar({
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-2xl border transition-all duration-300 shadow-sm ${isScoring
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-center">
          ...
          <div className="lg:col-span-4 space-y-2">
            <div className="flex items-center gap-2.5">
              <div
                className={`w-9 h-9 rounded-xl flex items-center justify-center shadow-md transition-transform ${isScoring
              ...
              <div>
                <h3
                  className={`text-base font-black tracking-tight ${isScoring
                      ? 'text-white'
            ...
            <div className="flex items-center gap-2 pt-1">
              <span
                className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-[11px] font-bold border ${isScoring
          ...
          <div className="lg:col-span-5 grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div
              className={`rounded-xl p-3 border transition-colors ${isScoring
            ...
            {/* Toggle Auto Reject */}
            <div
              className={`rounded-xl p-3 border transition-all ${autoReject
            ...
            >
              <label className="flex items-center justify-between gap-2 cursor-pointer h-full">
                <div className="flex items-start gap-1.5">
                  <ShieldAlert
                    className={`w-4 h-4 shrink-0 mt-0.5 ${autoReject ? 'text-rose-600' : 'text-slate-400'
                      }`}
                  />
                  <div>
                ...
                <button
                  ...
                  className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors shrink-0 ${autoReject ? 'bg-rose-600' : 'bg-slate-300 dark:bg-slate-700'
                    }`}
                >
                  <span
                    className={`inline-block h-3.5 w-3.5 rounded-full bg-white shadow transition-transform ${autoReject ? 'translate-x-4' : 'translate-x-1'
                      }`}
                  />
                </button>
              </label>
          ...
          <div className="lg:col-span-3 flex flex-col justify-center gap-2">
            <Button
              ...
              disabled={!canRun}
              className={`w-full py-3 rounded-xl font-bold text-xs shadow-md transition-all flex items-center justify-center gap-2 cursor-pointer ${isScoring
            ...
            >
              {isScoring ? (
                ...
                <>
                  ...
                  <span>Chấm điểm AI đơn mới ({unscoredCount} CV)</span>
                </>
              ) : !hasEnoughCredit ? (
                <>
                ...
                <>
                  <Zap className="w-4 h-4 text-amber-300" />
                  <span>Chấm lại các đơn ({totalCount} CV)</span>
                </>
              )}
            </Button>
