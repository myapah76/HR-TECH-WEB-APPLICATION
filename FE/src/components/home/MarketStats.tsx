...
export default function MarketStats({
  ...
  const stats = [
    ...
    },
    {
      ...
      iconBg: 'bg-indigo-50/60 dark:bg-indigo-950/20 border border-indigo-100/50 dark:border-indigo-900/20',
    },
    {
      label: 'Lượt ứng tuyển thành công',
      value: totalApplications,
      icon: <UserCheck className="h-6 w-6 text-emerald-600 dark:text-emerald-450" />,
      iconBg: 'bg-emerald-50/60 dark:bg-emerald-950/20 border border-emerald-100/50 dark:border-emerald-900/20',
    },
  ]
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {stats.map((stat, index) => (
            ...
            >
              <div className={`p-3.5 rounded-2xl flex items-center justify-center shrink-0 ${stat.iconBg}`}>
                {stat.icon}
              </div>
              <div>
        ...
        >
          ...
          <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-8 divide-y md:divide-y-0 md:divide-x divide-amber-800/40" id="newsletter-content">
