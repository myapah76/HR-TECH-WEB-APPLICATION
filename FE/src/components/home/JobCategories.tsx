        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6" id="categories-lists-grid">
          {categories.map((cat) => (
            ...
            >
              ...
              <div
                className={`px-5 py-4 border-b font-extrabold text-sm text-center flex items-center justify-center gap-2 ${cat.headerBg}`}
              ...
              <div className="divide-y divide-gray-100" id={`cat-rows-${cat.id}`}>
                {cat.jobs.map(
                  (item: { title: string; company: string; link: string }, index: number) => (
                    <div
                      key={index}
