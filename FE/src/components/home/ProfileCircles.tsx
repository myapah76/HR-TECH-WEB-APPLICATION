...
export default function ProfileCircles() {
  const circles = [
    ...
    },
    {
      ...
      image:
        'https://images.unsplash.com/photo-1451187580459-43490279c0fa?auto=format&fit=crop&w=600&h=600&q=80',
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6" id="circles-grid">
          {circles.map((item) => (
            ...
            >
              ...
              />
              <div className="absolute inset-0 bg-linear-to-t from-black/90 via-black/45 to-transparent z-10 transition-opacity duration-300"></div>

              {/* Badges and tags */}
              <div
                className="absolute top-4 left-4 z-20 flex flex-col gap-1.5"
                id={`circle-badges-${item.id}`}
              >
                <span
                  className={`text-[9px] font-black tracking-widest text-white uppercase px-2 py-0.5 rounded-sm shadow-xs ${item.badgeBg}`}
                >
                  {item.badge}
                </span>
              </div>
              ...
              <div
                className="absolute inset-x-0 bottom-0 z-20 p-5 text-white flex flex-col justify-end"
                id={`circle-content-${item.id}`}
              >
                <h4
                  className="text-sm font-black text-white group-hover:text-blue-300 transition-colors tracking-tight flex items-center justify-between"
                  id={`circle-title-${item.id}`}
                >
                  <span>{item.title}</span>
                  <ExternalLink className="h-3.5 w-3.5 opacity-0 group-hover:opacity-100 transition-opacity text-blue-300 shrink-0" />
                </h4>
                <p
                  className="text-[10px] font-medium text-gray-300 mt-1 line-clamp-2 leading-relaxed"
                  id={`circle-desc-${item.id}`}
                >
                  {item.subtitle}
                </p>
              </div>
