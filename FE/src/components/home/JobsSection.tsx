...
const mapBackendJobToUiJob = (bj: any): Job => {
  ...
  if (bj.jobType) {
    const typeLabel =
      bj.jobType === 'FULL_TIME' ? 'Fulltime' : bj.jobType === 'PART_TIME' ? 'Parttime' : bj.jobType
    tags.push(typeLabel)
  }
...
const JobCardSkeleton = () => (
  <div className="bg-white border border-slate-200/70 p-5 rounded-3xl flex gap-4 items-start animate-pulse h-40">
...
export default function JobsSection({ onJobSelect, onViewAllLatestClick }: JobsSectionProps) {
                  ...
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="flex gap-1 shrink-0">
                      {job.tags?.map((tag: string) => (
                        <span
                          key={tag}
                          className={`text-[9px] font-extrabold px-2.5 py-0.75 bg-slate-100 text-slate-655 rounded-lg uppercase tracking-wide`}
