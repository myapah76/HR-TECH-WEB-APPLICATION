      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-200/60 p-6 shadow-xs">
          <div className="flex items-center justify-between mb-5">
            ...
            <Link href="/candidate/applied-jobs" className="text-sm font-bold text-blue-600 hover:text-blue-800 flex items-center gap-1">
              Xem tất cả <ArrowRight className="h-4 w-4" />
            </Link>
          </div>
          <div className="space-y-3">
            {recentActivity.map((item, i) => (
              <div key={i} className="flex items-center gap-3.5 p-3 rounded-xl hover:bg-slate-50 transition-colors">
                <div className={`h-10 w-10 rounded-lg flex items-center justify-center shrink-0 ${
                  item.status === 'submitted' ? 'bg-blue-50 text-blue-600' :
                  item.status === 'viewed' ? 'bg-emerald-50 text-emerald-600' :
                  item.status === 'matched' ? 'bg-violet-50 text-violet-600' : 'bg-rose-50 text-rose-600'
                }`}>
                  {item.status === 'submitted' ? <Send className="h-4.5 w-4.5" /> :
                   item.status === 'viewed' ? <Eye className="h-4.5 w-4.5" /> :
                   item.status === 'matched' ? <Brain className="h-4.5 w-4.5" /> : <Heart className="h-4.5 w-4.5" />}
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-400 font-bold flex items-center gap-1 mt-0.5">
                    <Clock className="h-4 w-4" />{item.time} trước
                  </p>
                </div>
              </div>
            ))}
        ...
        <div className="space-y-5">
          <div className="bg-white rounded-2xl border border-slate-200/60 p-5 shadow-xs">
            ...
            <div className="space-y-2">
              <Link href="/candidate/resume-builder" className="flex items-center gap-3 p-3 rounded-xl hover:bg-blue-50 transition-colors group">
                <FileText className="h-5 w-5 text-blue-600" />
                <span className="text-sm font-bold text-slate-700 group-hover:text-blue-600">Tạo CV mới</span>
              </Link>
              <Link href="/candidate/ai-advisor" className="flex items-center gap-3 p-3 rounded-xl hover:bg-violet-50 transition-colors group">
                <Brain className="h-5 w-5 text-violet-600" />
                <span className="text-sm font-bold text-slate-700 group-hover:text-violet-600">Quét CV bằng A.I</span>
              </Link>
              <Link href="/jobs" className="flex items-center gap-3 p-3 rounded-xl hover:bg-emerald-50 transition-colors group">
                <Briefcase className="h-5 w-5 text-emerald-600" />
                <span className="text-sm font-bold text-slate-700 group-hover:text-emerald-600">Tìm việc làm</span>
              </Link>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl p-5 text-white">
            ...
            <Link href="/candidate/profile" className="mt-3 inline-block bg-white text-blue-700 font-black text-sm py-2.5 px-5 rounded-lg hover:bg-blue-50 transition-colors">
              Cập nhật ngay
            </Link>
          </div>
        </div>
