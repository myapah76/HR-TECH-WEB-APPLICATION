...
import { JobMatchingTaskResponse, JobRecommendationResponse } from '@/src/types/recommendation.type'
import { Star, UploadCloud, FileText, Loader2, CheckCircle2, AlertCircle, Map, Layers, FileSearch } from 'lucide-react'
...
export default function RecommendJobsPage() {
  ...
  const getStatusIcon = (status: string | undefined) => {
    switch (status) {
      case 'PENDING': return <Loader2 className="w-8 h-8 text-slate-400 animate-spin" />
      case 'EXTRACTING': return <FileSearch className="w-8 h-8 text-amber-500 animate-pulse" />
      case 'MAPPING': return <Map className="w-8 h-8 text-blue-500 animate-pulse" />
      case 'SCORING': return <Layers className="w-8 h-8 text-indigo-500 animate-pulse" />
      case 'DONE': return <CheckCircle2 className="w-8 h-8 text-emerald-500" />
      case 'FAILED': return <AlertCircle className="w-8 h-8 text-rose-500" />
      default: return <Loader2 className="w-8 h-8 text-slate-400" />
    }
  }
  ...
  const getStatusColor = (status: string | undefined) => {
    switch (status) {
      ...
      case 'SCORING': return 'bg-indigo-500'
    ...
    <div className="space-y-8 animate-fade-in max-w-6xl mx-auto">
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-3xl font-black tracking-tight text-slate-900">AI Cố vấn: Gợi ý Việc làm</h1>
        </div>
        <p className="text-slate-500 font-medium">Sử dụng công nghệ Graph & LLM để tìm kiếm và đề xuất những công việc phù hợp nhất với kỹ năng trong CV của bạn.</p>
      </div>
      ...
      {isDone && taskStatus?.recommendedJobs ? (
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-700">
          <div className="flex items-center justify-between">
            ...
            <Button variant="outline" onClick={() => { setTaskId(null); setTaskStatus(null); }} className="font-bold border-blue-200 text-blue-700 hover:bg-blue-50">
              Tìm kiếm lại
            </Button>
          </div>
          ...
          {taskStatus.recommendedJobs.length === 0 ? (
            <div className="p-12 text-center bg-white rounded-xl border border-slate-200 shadow-sm">
              <p className="text-slate-500 font-medium text-lg">Rất tiếc, chưa tìm thấy công việc nào phù hợp với kỹ năng của bạn lúc này.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {taskStatus.recommendedJobs.map((rec, idx) => (
                <Card key={idx} className="border-slate-200 hover:border-blue-300 hover:shadow-lg transition-all bg-white overflow-hidden group">
                    ...
                    <div className="flex justify-between items-start gap-4">
                      <div>
                        <Link href={`/jobs/${rec.jobId}`} className="hover:underline">
                          <CardTitle className="text-lg text-blue-900 leading-tight">{rec.jobTitle}</CardTitle>
                        </Link>
                        <CardDescription className="mt-1 font-medium text-slate-600">{rec.companyName}</CardDescription>
                      </div>
                      <div className="flex flex-col items-end">
                        <div className="bg-emerald-100 text-emerald-700 px-3 py-1 rounded-full text-sm font-black flex items-center gap-1 shadow-sm">
                          {(rec.matchScore * 100).toFixed(0)}% <span className="text-xs font-semibold">Phù hợp</span>
                        </div>
                      </div>
                    </div>
                  </CardHeader>
                  <CardContent className="p-5 space-y-4">
                    ...
                    <div>
                      <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Kỹ năng đáp ứng</div>
                      <div className="flex flex-wrap gap-1.5">
                        {rec.matchedSkills?.length > 0 ? rec.matchedSkills.slice(0, 5).map(skill => (
                          <span key={skill} className="px-2 py-0.5 bg-blue-50 text-blue-700 text-xs font-semibold rounded border border-blue-100">
                            {skill}
                          </span>
        ...
        /* UPLOAD & MOCKUP UI */
        <div className={`grid grid-cols-1 ${isProcessActive ? 'lg:grid-cols-2' : ''} gap-8 items-start transition-all duration-700`}>
          {/* Left: Form */}
          <Card className={`border-blue-100 shadow-lg bg-white overflow-hidden relative z-10 transition-all duration-500 ${!isProcessActive ? 'max-w-2xl mx-auto w-full' : ''}`}>
            ...
            <CardContent className="pt-6">
                ...
                <div className="space-y-4">
                  ...
                  {loadingCvs ? (
                    ...
                    >
                      ...
                      {cvs.map(cv => (
                        <option key={cv.id} value={cv.id}>{cv.title} {cv.isPrimary ? '(Mặc định)' : ''}</option>
                      ))}
                    </select>
                  ) : (
              ...
              <div className="mt-8 pt-6 border-t border-slate-100">
                <Button 
                  ...
                  disabled={isStarting || isProcessActive || (cvMode === 'existing' && !selectedCvId) || (cvMode === 'new' && (!selectedFile || !cvTitle))}
                ...
                >
                  {isStarting || isProcessActive ? <Loader2 className="w-6 h-6 animate-spin" /> : <Star className="w-6 h-6" />}
          ...
          {isProcessActive && (
            <div className="relative flex flex-col items-center justify-center p-8 bg-slate-100/50 rounded-2xl border border-slate-200 shadow-inner min-h-[500px] overflow-hidden animate-in fade-in slide-in-from-right-8 duration-500">
               ...
               <div className="relative w-full max-w-sm aspect-[1/1.4] rounded-lg shadow-2xl overflow-hidden border border-slate-300 bg-white">
                 <img src="/cv_mockup.png" alt="CV Mockup" className="w-full h-full object-cover opacity-90" />
                      ...
                      <div className="flex items-center gap-3 mb-4">
                        ...
                        <div className="flex-1">
                          <div className="flex justify-between items-center mb-1">
                             <span className="font-bold text-sm text-slate-800">{taskStatus.message}</span>
                             <span className="font-black text-blue-700">{taskStatus.progressPercentage}%</span>
                          </div>
                          <Progress value={taskStatus.progressPercentage} className="h-2 bg-blue-100" indicatorColor={getStatusColor(taskStatus.status)} />
                        </div>
                      </div>

                      <div className="grid grid-cols-4 gap-1 w-full opacity-80">
                        <div className={`text-[10px] font-bold text-center p-1.5 rounded ${taskStatus.progressPercentage >= 20 ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-400'}`}>Bóc tách</div>
                        <div className={`text-[10px] font-bold text-center p-1.5 rounded ${taskStatus.progressPercentage >= 50 ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-400'}`}>Ánh xạ</div>
                        <div className={`text-[10px] font-bold text-center p-1.5 rounded ${taskStatus.progressPercentage >= 80 ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-100 text-slate-400'}`}>Graph</div>
                        <div className={`text-[10px] font-bold text-center p-1.5 rounded ${taskStatus.progressPercentage === 100 ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-400'}`}>Xong</div>
                      </div>
                  </div>
               )}
            </div>
