...
const jobSchema = z.object({
  ...
  salaryMin: z.coerce.number().min(0, 'Lương tối thiểu không hợp lệ').optional(),
  salaryMax: z.coerce.number().min(0, 'Lương tối đa không hợp lệ').optional(),
  ...
  const onSubmit = (data: JobFormValues) => {
    ...
    createJobMutation.mutate(payload, {
      onSuccess: () => {
        ...
        router.push('/company/manage-jobs') // Assuming you have this route
      },
      onError: (err: any) => {
        toast.error(err?.response?.data?.message || 'Có lỗi xảy ra khi tạo tin tuyển dụng')
      },
    })
  }
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          ...
          <div className="space-y-6">
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1">Tên công việc (Title) <span className="text-red-500">*</span></label>
              <input
                type="text"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1">Cấp bậc <span className="text-red-500">*</span></label>
                <select
                  className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all bg-white"
            ...
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1">Địa điểm làm việc <span className="text-red-500">*</span></label>
              <input
                type="text"
                className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all"
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1">Lương tối thiểu (VND)</label>
              <input
                type="number"
                className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all"
            ...
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1">Lương tối đa (VND)</label>
              <input
                type="number"
                className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all"
            ...
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1">Hạn nộp hồ sơ <span className="text-red-500">*</span></label>
              <input
                type="date"
                className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all"
                {...register('deadline')}
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          ...
          <div className="space-y-6">
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1">Mô tả công việc <span className="text-red-500">*</span></label>
              <textarea
                rows={6}
            ...
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1">Yêu cầu ứng viên <span className="text-red-500">*</span></label>
              <textarea
                rows={6}
                className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all resize-y"
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          ...
          <div className="space-y-8">
            {/* REQUIRED SKILLS */}
            <div>
              <label className="block text-base font-bold text-slate-900 mb-1">Kỹ năng bắt buộc (Required Skills) <span className="text-red-500">*</span></label>
              <p className="text-sm text-slate-500 mb-4">Các kỹ năng trọng yếu và mức độ trình độ bắt buộc ứng viên phải có.</p>
              <RequiredSkillInput 
            ...
            {/* RELATED SKILLS */}
            <div>
              <label className="block text-base font-bold text-slate-900 mb-1">Kỹ năng liên quan (Related Skills)</label>
              <p className="text-sm text-slate-500 mb-4">Các kỹ năng phụ trợ, điểm cộng thêm, không yêu cầu mức độ cụ thể.</p>
              <SkillTagInput 
                value={relatedSkills} 
