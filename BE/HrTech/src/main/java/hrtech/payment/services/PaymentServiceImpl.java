...
public class PaymentServiceImpl implements IPaymentService {
    ...
    public CreatePaymentResponse createPayment(CreatePaymentRequest request) {
        ...
        var pendingPaymentOpt = paymentRepository.findFirstByUserIdAndStatusOrderByCreatedAtDesc(user.getId(),
        ...
        if (pendingPaymentOpt.isPresent()) {
            ...
            try {
                ...
                PaymentLink paymentInfo = payOSService.getPayOSClient().paymentRequests()
                        .get(pendingPayment.getOrderCode());
                ...
                if (payosStatus == PaymentLinkStatus.PAID) {
                    ...
                    // Nếu trên PayOS vẫn đang pending -> Kiểm tra xem subscriptionPlanId có trùng
                    // với đơn hàng cũ không
                    ...
                    // Nếu trên PayOS đã bị hủy hoặc hết hạn -> Cập nhật trạng thái để giải phóng
                    // khóa
                ...
                // Nếu không tìm thấy đơn hàng trên PayOS hoặc có lỗi kết nối, giải phóng đơn
                // hàng cũ để cho phép tạo mới
                pendingPayment.setStatus(PaymentStatus.CANCELLED);
                paymentRepository.save(pendingPayment);
            }
        }

        // Kiểm tra tính hợp lệ của việc gia hạn/mua gói cước trước khi cho phép tạo QR
        // thanh toán
