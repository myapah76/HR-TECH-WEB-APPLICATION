...
public class SkillExtractionServiceImpl implements ISkillExtractionService {
        ...
        try {
            ...
            if (parseResult.getParsedContent() != null && !parseResult.getParsedContent().isBlank()) {
                ...
                if (cv.getUser() != null) {
                    ...
                    metadata.put("fullName", (cv.getUser().getFirstName() != null ? cv.getUser().getFirstName() : "")
                            + " " + (cv.getUser().getLastName() != null ? cv.getUser().getLastName() : ""));
                }
    ...
    public CompletableFuture<JobExtractionResponse> extractAndSaveJobSkills(UUID jobId) {
        ...
        try {
            ...
            fullJobText.append("Salary: ").append(job.getSalaryMin()).append(" - ").append(job.getSalaryMax())
                    .append("\n");
            fullJobText.append("Job Type: ").append(job.getJobType() != null ? job.getJobType().name() : "")
                    .append("\n");
            fullJobText.append("Experience Level: ")
                    .append(job.getExperienceLevel() != null ? job.getExperienceLevel().name() : "").append("\n\n");
                    ...
                    if (rel.getRelations() != null) {
                        for (SkillRelationDetail detail : rel.getRelations()) {
                            ...
                            if ("PARENT_OF".equals(type)) {
                                String parentName = (detail.getParent() != null && !detail.getParent().isBlank())
                                        ? detail.getParent()
                                        : newSkill;
                                String childName = (detail.getChild() != null && !detail.getChild().isBlank())
                                        ? detail.getChild()
                                        : detail.getTarget();

                                if (parentName == null || childName == null || parentName.equalsIgnoreCase(childName))
                                    continue;

                                Boolean alreadyLinked = skillNodeRepository.anyRelationshipExistsByName(parentName,
                                        childName);
                                if (Boolean.TRUE.equals(alreadyLinked)) {
                                    log.debug("Relationship between '{}' and '{}' already exists, skipping", parentName,
                                            childName);
                                    continue;
                                }
                                ...
                                if (target == null || target.equalsIgnoreCase(newSkill))
                                    continue;
                                Boolean alreadyLinked = skillNodeRepository.anyRelationshipExistsByName(newSkill,
                                        target);
                                if (!Boolean.TRUE.equals(alreadyLinked)) {
     ...
     *
     * Note: Short but valid skills like "Go", "C#", "C++" pass because they contain
     * letters
     * and are >= 2 chars. Single-char extractions like "r" (R language) are
     * unfortunately
